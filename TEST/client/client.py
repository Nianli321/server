...
def thread_check_server():
    while True:
        ...
        try:
            ...
            print("\nGoodBye server is shutting down")
            clientSocket.close()
            break
    os._exit(1)
...
while(1):
    ...
    if not check_command (comm):
        ...
        if len(comm.split()) == 3:
            ...
            reply = clientSocket.recv(2)
            if reply == b"NE":
                print("deleting failed, {} thread doesn't exist".format(comm.split()[0]))
            elif reply == b"NC":
                print("deleting rejected, {} is not the creater of message {}".format(username, comm.split()[0]))
            elif reply == b"NM":
                print("deleting failed, messgae doesn't exist")
            elif reply == b"OK":
        ...
        if len(comm.split()) >= 4:
            ...
            reply = clientSocket.recv(2)
            if reply == b"NE":
                print("editing failed, {} thread doesn't exist".format(comm.split()[1]))
            elif reply == b"NC":
                print("editing rejected, {} is not the creater of message {}".format(username, comm.split()[1]))
            elif reply == b"NM":
                print("deleting failed, messgae doesn't exist")
            elif reply == b"OK":
                print("message edted")            
            ...
            print("invalid parameter: Please follow the correct format \"EDT Thread MessageNumber Message\"")
